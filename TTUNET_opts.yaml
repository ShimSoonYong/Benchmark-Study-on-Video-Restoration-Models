...
class BiasFree_LayerNorm(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        sigma: Tensor = x.var(-1, keepdim=True, unbiased=False)
...
class WithBias_LayerNorm(nn.Module):
    def __init__(self, normalized_shape) -> None:
        ...
        if isinstance(normalized_shape, numbers.Integral):
            normalized_shape: tuple[numbers.Integral] = (normalized_shape,)
        normalized_shape = torch.Size(normalized_shape)
        ...
        self.normalized_shape: tuple[numbers.Integral] = normalized_shape

    def forward(self, x: Tensor) -> Tensor:
        mu: Tensor = x.mean(-1, keepdim=True)
        sigma: Tensor = x.var(-1, keepdim=True, unbiased=False)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim: int, num_heads: int, ffn_expansion_factor: float, bias: bool, LayerNorm_type: str) -> None:
